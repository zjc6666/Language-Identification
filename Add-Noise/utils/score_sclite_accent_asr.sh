...
# Changed by Zhang Jicheng at Nov 4, 2020
...
python calculate_accent_generate_asr_json.py ${dir}/data.json "post" ${dir}/accent_asr_data.json ${dir}/accent_recog.txt

cat ${dir}/accent_recog.txt | awk '{a+=1;if($5==$7) b+=1;}END{print "Accurate is : " b/a}' > ${dir}/accent_acc.txt

if [ $num_spkrs -eq 1 ]; then

#  json2trn.py ${dir}/data.json ${dic} --num-spkrs ${num_spkrs} --refs ${dir}/ref.trn --hyps ${dir}/hyp.trn
   json2trn.py ${dir}/accent_asr_data.json ${dic} --num-spkrs ${num_spkrs} --refs ${dir}/ref.trn --hyps ${dir}/hyp.trn
...
#  json2trn.py ${dir}/data.json ${dic} --num-spkrs ${num_spkrs} --refs ${ref_trns} --hyps ${hyp_trns}
  json2trn.py ${dir}/accent_asr_data.json ${dic} --num-spkrs ${num_spkrs} --refs ${ref_trns} --hyps ${hyp_trns}
