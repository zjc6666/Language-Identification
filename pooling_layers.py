...
class attentive_SE(nn.Module): #attentive squeeze and excitation
    def __init__(self, featdim=1024, reduction=4):
        super(attentive_SE, self).__init__()
        self.featdim = featdim
        self.factor = np.sqrt(featdim)
        self.lang_v = torch.nn.Parameter(torch.randn(featdim))
        self.transformation = nn.Sequential(nn.Linear(featdim, featdim // reduction, bias=False),
                                            nn.ReLU(inplace=True),
